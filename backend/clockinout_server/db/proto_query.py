...
class ProtoDBQuery:
    ...
    def __call__(self, dbsession: Session, 
        ...
        if is_proto_field_set(proto, "id"):
            tgtprop = _resolve_db_coltype_from_proto_field("id", self.dbtype)
            sqlexpr = tgtprop == proto.id
        ...
        query = dbsession.query(self.dbtype).filter(sqlexpr)
        query = self.qmanip(query)
...
class ProtoDBItem:
    ...
    def remove(self, dbsession: Session, proto:ItemRequest) -> None:
        itemdesc = self._incoming_request_common(proto)
    ...
    def modify(self, dbsession: Session, proto: ItemRequest) -> None:
        itemdesc = self._incoming_request_common(proto)
        dbitem = ProtoDBQuery(self.dbtype, return_only_one=True)(dbsession, itemdesc)
        if not dbitem:
            raise ValueError("couldn't find matching item in database")
...
class ProtoDBOrg(ProtoDBItem):
    ...
    def _new_item_hook(self, dbsession: Session, dbobj: Org, request: OrgInfo):
        ...
        if request.admin_user == UserInfo():
            raise ValueError("must supply an admin user to create a new organisation")
        admin_user_lookup = ProtoDBQuery(User, return_only_one=True)(dbsession, request.admin_user)
        if not admin_user_lookup:
            raise ValueError("couldn't find a matching admin user in database")
        if not admin_user_lookup.hashed_pw:
            raise ValueError("org admin user must have a password associated")
